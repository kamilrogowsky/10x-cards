...
export const Homepage: React.FC<HomepageProps> = ({ user }) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Coś poszło nie tak</h2>
  ...
  return (
    <>
      {/* Top Bar */}
      <TopBar cta={data?.cta} isLoading={isLoading} user={user} />

      <main className="min-h-screen">
        {/* Hero Section */}
        <div id="hero" className="bg-gradient-to-b from-background via-background to-muted/20 scroll-mt-4 pt-16">
          <HeroSection appInfo={data?.app_info || { name: "", description: "", version: "" }} isLoading={isLoading} />
        </div>

        {/* Features Section */}
        <div id="features" className="bg-background scroll-mt-4">
          <FeaturesSection features={data?.features || []} isLoading={isLoading} />
        </div>
        ...
        {error && data && (
          <div className="fixed bottom-4 right-4 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-200 px-4 py-3 rounded-lg shadow-lg animate-slide-up backdrop-blur-sm">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium">Niektóre dane mogą być nieaktualne. Używamy zapisanej wersji.</p>
            </div>
          </div>
        )}
      </main>
